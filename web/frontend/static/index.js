...
async function createProject(e) {
    ...
    try {
        var resp = await fetch('/api/v1/projects', {method: 'POST', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({title: title, platform: platform, length: length, one_liner: oneLiner, genre_tags: genreTags.split(',').map(function(t) { return t.trim(); }).filter(Boolean), target_words_min: parseInt(minWords) || null, target_words_max: parseInt(maxWords) || null})});
...
async function saveProjectEdit() {
    ...
    try {
        await fetch('/api/v1/projects/' + _editProjectId, {method: 'PATCH', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({title: document.getElementById('edit-title').value, one_liner: document.getElementById('edit-one-liner').value, platform: document.getElementById('edit-platform').value, length: document.getElementById('edit-length').value, genre_tags: document.getElementById('edit-genre-tags').value.split(',').map(function(t) { return t.trim(); }).filter(Boolean), target_words_per_chapter: parseInt(document.getElementById('edit-target-words').value) || null})});
