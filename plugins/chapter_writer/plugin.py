...
## 五、章尾四式（每次选一种）
1. **悬崖式**：刀劈到面门。门被踹开。声音戛然而止。
2. **揭秘式**：读者和主角同时知道了一个新信息——原来XXX是XXX。
3. **反讽式**：读者知道了主角不知道的信息——看着主角走向陷阱。
4. **情绪式**：不抛事件钩子，用一句不动声色的话让读者心里一颤。
...
class ChapterWriterPlugin:
    ...
    def _is_fanqie_expert_mode(self, platform: str) -> bool:
        ...
        if platform != "fanqie":
            return False
        return self._kernel.get_config("chapter.fanqie_expert.enabled", False)
